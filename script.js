...
document.addEventListener('DOMContentLoaded', function() {
  // Get elements with new class/ID names
  const progressFill = document.querySelector('.crybug-progress-fill');
  const stepIndicator = document.getElementById('crybug-step-indicator');
  ...
  const progressSection = document.getElementById('crybug-progress-section');
  const progressBar = document.getElementById('crybug-progress-bar');
  ...
  function updateProgressBar() {
    ...
    // When section bottom is at viewport bottom, progress is 100%
    const visibleHeight = Math.min(viewportHeight, sectionHeight);
    let scrollProgress = 0;
    
    if (sectionTop <= 0) {
      // Section's top is above viewport top
      if (sectionTop <= viewportHeight - sectionHeight) {
        // Section is completely scrolled past
        scrollProgress = 100;
      } else {
        // Section is partially visible
        scrollProgress = (-sectionTop / (sectionHeight - visibleHeight)) * 100;
      }
    }
    ...
    // Show or hide progress elements based on section visibility
    const isSectionVisible = 
      sectionRect.top < viewportHeight && 
      sectionRect.bottom > 0;
      
    progressBar.style.opacity = isSectionVisible ? '1' : '0';
    
    // Only show step indicator when scrolled a bit into the section
    if (isSectionVisible && -sectionTop > 200) {
  ...
  window.addEventListener('scroll', function() {
    ...
    // Check which cards are in view and update dots
    processCards.forEach(card => {
      const rect = card.getBoundingClientRect();
      const stepNum = card.getAttribute('data-step');
      const dot = document.querySelector(`.crybug-step-dot[data-step="${stepNum}"]`);
  ...
  expandBtns.forEach(btn => {
    btn.addEventListener('click', function(e) {
      ...
      if (content.classList.contains('expanded')) {
        ...
        // Delayed hide for smooth animation
        setTimeout(() => {
          content.classList.add('hidden');
        }, 500);
      } else {
        content.classList.remove('hidden');
        // Small delay to ensure the hidden class is removed before adding expanded
        setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    function highlightNextCard() {
      if (index >= processCards.length) {
        // Reset all cards after the sequence
      ...
      const stepNum = processCards[index].getAttribute('data-step');
      stepDots.forEach(dot => dot.classList.remove('active'));
      document.querySelector(`.crybug-step-dot[data-step="${stepNum}"]`).classList.add('active');
      
      // Move to next card
      index++;
      setTimeout(highlightNextCard, 500);
    }
  ...
  stepDots.forEach(dot => {
    dot.addEventListener('click', function() {
      const stepNum = this.getAttribute('data-step');
      const targetCard = document.querySelector(`.crybug-process-card[data-step="${stepNum}"]`);
  ...
  document.querySelectorAll('.crybug-fade-up').forEach(el => {
    observer.observe(el);
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  timelineItems.forEach(item => {
    item.addEventListener('click', function(e) {
      ...
      // Log the step (would connect to analytics in a real app)
      console.log(`User clicked on step ${stepNumber}`);
    });
  });
  
  // Add click event listener to the document to close expanded item when clicking outside
  document.addEventListener('click', function(e) {
    // If there's an expanded item and the click wasn't on a timeline item
    if (currentExpandedItem && !e.target.closest('.timeline-item')) {
      // Remove highlight from the expanded item
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => {
    animateCounter('stat1', 1200, 1500, '+');
    animateCounter('stat2', 87, 1500, '%');
    animateCounter('stat3', 34500, 2000, '+');
    animateCounter('stat4', 570, 1500, '+');
  }, 500);
...
document.addEventListener('DOMContentLoaded', function() {
  const toggles = document.querySelectorAll('.crybug-faq-toggle');
  ...
  toggles.forEach(toggle => {
    toggle.addEventListener('click', () => {
      ...
      if (content.style.maxHeight) {
        ...
        document.querySelectorAll('.crybug-faq-content').forEach(c => c.style.maxHeight = null);
        document.querySelectorAll('.crybug-faq-toggle i').forEach(i => i.style.transform = 'rotate(0deg)');
